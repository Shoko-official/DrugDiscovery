...
fn complete_event() -> DecisionEvent {
    DecisionEvent {
        ...
        decision: Some(DecisionRecord {
            ...
            decision_criterion: Some(decision_criterion()),
        }),
    }
}
...
fn json_node_count(value: &serde_json::Value) -> usize {
    1 + match value {
        serde_json::Value::Array(values) => values.iter().map(json_node_count).sum(),
        serde_json::Value::Object(values) => values.values().map(json_node_count).sum(),
        _ => 0,
    }
}
...
fn projects_canonical_payload_and_round_trips_without_loss() {
    ...
    assert_eq!(row.payload["ood_status"], json!("borderline"));
    assert_eq!(
        row.payload["ood_detector"],
        json!({
    ...
    assert_eq!(
        row.payload["decision_criterion"],
        json!({
            "criterion_id": "potency_policy",
            "criterion_version": "2026.07",
    ...
    assert_eq!(
        row.payload_sha256,
        "692da3fee27b32d4d7b24132dd6fd96bdb14280783a2ca18ea49dbce4d505d9f"
    );
    assert_eq!(
...
#[test]
fn projects_and_reconstructs_every_decision_criterion_comparator() {
    for (comparator, canonical) in [
        (DecisionCriterionComparator::LessThan, "less_than"),
        (
            DecisionCriterionComparator::LessThanOrEqual,
            "less_than_or_equal",
        ),
        (DecisionCriterionComparator::GreaterThan, "greater_than"),
        (
        ...
        assert_eq!(row.payload["decision_criterion"]["comparator"], canonical,);
        assert_eq!(reconstruct_decision_event(&row).unwrap(), event);
    }
}
...
fn maximum_prediction_positions_and_rationales_fit_the_canonical_node_envelope() {
    ...
    assert_eq!(json_node_count(&row.payload), 127);
...
fn round_trips_every_supported_ood_status() {
    for (ood_status, recommendation, canonical_ood_status, canonical_recommendation) in [
        (
            OodStatus::InDomain,
            Recommendation::StopProgram,
            "in_domain",
        ...
        let mut event = complete_event();
        let decision = event.decision.as_mut().unwrap();
        decision.ood_status = Some(ood_status as i32);
        decision.recommendation = recommendation as i32;
...
#[test]
fn historical_out_of_domain_non_abstain_event_remains_readable_without_mutation() {
    const HISTORICAL_PAYLOAD_SHA256: &str =
...
#[test]
fn historical_m34_payload_without_decision_criterion_reconstructs_without_mutation() {
    const M34_PAYLOAD_SHA256: &str =
        "1d8a4aa0f60fb75fcca44744b07c0c85e2e81974ed95d7eb7926c029ab47f655";

    let row = row_with_literal_payload(M34_CANONICAL_PAYLOAD_WITHOUT_CRITERION, M34_PAYLOAD_SHA256);
    assert_eq!(
        format!(
            "{:x}",
            Sha256::digest(M34_CANONICAL_PAYLOAD_WITHOUT_CRITERION)
        ),
        M34_PAYLOAD_SHA256
    );
    let original = row.clone();
    ...
    assert!(decision.prediction_interval.is_some());
    assert_eq!(decision.prediction_positions.len(), 2);
...
fn rejects_unqualified_ood_status_before_persistence_with_redacted_errors() {
    for (invalid, missing) in [
        (None, true),
        (Some(OodStatus::Unspecified as i32), false),
        (Some(OodStatus::Unknown as i32), false),
        (Some(i32::MAX), false),
    ] {
        let mut event = complete_event();
        ...
        assert!(matches!(
            (&error, missing),
            (EventProjectionError::MissingOodStatus, true)
                | (EventProjectionError::UnqualifiedOodStatus, false)
        ));
        let rendered = format!("{error:?} {error}");
        assert!(!rendered.contains(&i32::MAX.to_string()));
    }
}

#[test]
fn rejects_missing_or_invalid_ood_detector_before_persistence() {
    let mut missing = complete_event();
    missing.decision.as_mut().unwrap().ood_detector = None;
    assert!(matches!(
        project_decision_event(missing, metadata()),
        Err(EventProjectionError::MissingOodDetector)
    ));

    for (detector_id, detector_version) in [
        ("", "model-2026.07"),
        ("mahalanobis", ""),
        (" mahalanobis", "model-2026.07"),
        ("mahalanobis", "model-2026.07 "),
        ("mahalanobis\0secret", "model-2026.07"),
    ] {
        let mut event = complete_event();
        event.decision.as_mut().unwrap().ood_detector = Some(OodDetectorRef {
            detector_id: detector_id.to_owned(),
            detector_version: detector_version.to_owned(),
        });

        let error = project_decision_event(event, metadata()).unwrap_err();
        assert!(matches!(error, EventProjectionError::InvalidDecision(_)));
        let rendered = format!("{error:?} {error}");
        if !detector_id.is_empty() {
            assert!(!rendered.contains(detector_id));
        }
        if !detector_version.is_empty() {
...
#[test]
fn rejects_missing_decision_criterion_before_persistence_with_a_fixed_error() {
    let mut event = complete_event();
    event.decision.as_mut().unwrap().decision_criterion = None;

    let error = project_decision_event(event, metadata()).unwrap_err();

    assert!(matches!(
        &error,
        EventProjectionError::MissingDecisionCriterion
    ));
    assert_eq!(
        error.to_string(),
        "new decision events require a decision criterion"
    );
}
...
#[test]
fn rejects_out_of_domain_recommendation_other_than_abstain() {
    for recommendation in [
        Recommendation::Promote,
        Recommendation::Reject,
        Recommendation::Defer,
        ...
        assert!(matches!(
            project_decision_event(event, metadata()),
            Err(EventProjectionError::OutOfDomainRequiresAbstain)
        ));
    }
}

...
test("builds PostgreSQL integration tests without credentials and runs them only beside PostgreSQL", async () => {
  ...
  assert.match(sourceCommand, /crates\/decision-grpc-client\/Cargo\.toml/);
  assert.match(sourceCommand, /crates\/decision-grpc-client\/src\/lib\.rs/);
  assert.match(
    sourceCommand,
    /crates\/decision-grpc-client\/tests\/access_token\.rs/,
  );
  assert.match(
    sourceCommand,
    /crates\/decision-grpc-client\/tests\/client_config\.rs/,
  );
  assert.match(
    sourceCommand,
    /crates\/decision-grpc-client\/tests\/client_transport\.rs/,
  );
  assert.match(
    sourceCommand,
    /crates\/desktop-decision-grpc\/Cargo\.toml/,
  );
  assert.match(
    sourceCommand,
    /crates\/desktop-decision-grpc\/src\/lib\.rs/,
  );
  assert.match(
    sourceCommand,
    /crates\/desktop-decision-grpc\/src\/tests\.rs/,
  );
  assert.match(sourceCommand, /crates\/decision-grpc-server\/Cargo\.toml/);
  assert.match(sourceCommand, /crates\/decision-grpc-server\/src\/lib\.rs/);
  assert.match(sourceCommand, /crates\/decision-grpc-server\/src\/config\.rs/);
  assert.match(sourceCommand, /crates\/decision-grpc-server\/src\/server\.rs/);
  assert.match(
    sourceCommand,
    /crates\/decision-grpc-server\/tests\/server_config\.rs/,
  );
  assert.match(
    sourceCommand,
    /crates\/decision-grpc-server\/tests\/server_startup\.rs/,
  );
  assert.match(
    sourceCommand,
    /crates\/decision-grpc-server\/tests\/server_transport\.rs/,
  );
